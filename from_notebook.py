...
floats = list(
    df_events.select_dtypes(include=["float64"]).columns
...
df_events = df_events.drop(
    columns=["deadline_time", "release_time"]
)  # DROP IRRELEVANT COLUMNS

df_events["deadline_time_epoch"] = pd.to_datetime(
    df_events["deadline_time_epoch"], unit="s"
)  # CONVERT TO DATETIME
...
sns.lineplot(
    data=df_events, x=df_events["name"], y=df_events["average_entry_score"], ax=ax
)
...
def get_manager_info(manager_id):
    ...
    transfers_info["time"] = (
        pd.to_datetime(transfers_info["time"]).dt.tz_localize(None).dt.date
    )
    transfers_number = pd.DataFrame(
        transfers_info.groupby("event")
        .count()
        .element_in.rename("transfers_per_gameweek")
    )
    transfers_number

    url_team_gw = (
        f"https://fantasy.premierleague.com/api/entry/{manager_id}/event/1/picks/"
    )
    ...
    team_gw_columns = [
        ...
        "entry_history.rank",
        "entry_history.rank_sort",
        "entry_history.overall_rank",
        "entry_history.percentile_rank",
        "entry_history.bank",
        "entry_history.value",
    ...
    for i in range(
        2, 27
    ):  # NALEŻY ZNALEŹĆ AUTOMATYZACJĘ ILOŚCI GW + JAK WPISZESZ ZA DUŻĄ WARTOŚĆ JEST KAPLICA
        url_team_new_gw = (
            f"https://fantasy.premierleague.com/api/entry/{manager_id}/event/{i}/picks/"
        )
...
def manager_team_gw(manager_id, gameweek):
    ...
    for i in range(1, 16):
        ...
        team_gw_df["points"].loc[team_gw_df.index == i] = int(
            df_stats.loc[df_stats.index == gameweek].total_points
        )
...
# _______________________________________________________________________________________________________________________
# VIII. GET PERFORMANCE PREDICTION FOR MANAGER'S TEAM ON CHOSEN GAMEWEEK
def team_prediction(manager_id):
    ...
    team_gw_df = team_gw_df.join(
        df_players_reindex["chance_of_playing_next_round"],
        how="left",
        on=team_gw_df.element,
    )
